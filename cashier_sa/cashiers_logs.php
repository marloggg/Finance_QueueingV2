...
$sql = "SELECT COUNT(*) FROM `queue_list_sa` ql JOIN `cashier_list` c ON ql.cashier_id = c.cashier_id JOIN `teller_list` t ON ql.teller_id = t.teller_id";
...
$sql = "SELECT ql.*, c.*, t.* FROM `queue_list_sa` ql JOIN `cashier_list` c ON ql.cashier_id = c.cashier_id JOIN `teller_list` t ON ql.teller_id = t.teller_id WHERE c.lastname = '" . $_SESSION['lastname'] ."' ORDER BY ql.date_created ASC LIMIT $start, $limit";
$qry = $conn->query($sql);
$i = $start + 1;
