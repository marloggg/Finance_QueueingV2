...
<div class="container-fluid">
    <form action="" id="cashier-form">
        ...
        <div class="form-group">
            <label for="log_status" class="control-label">Activity</label>
            <select name="log_status" id="log_status" class="form-select form-select-sm rounded-0" required>
                <option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>In-Use</option>
                <option value="2" <?php echo isset($status) && $status == 2 ? 'selected' : '' ?>>Not In-Use</option>
            </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="text" name="email_address" id="email_address" required class="form-control form-control-sm rounded-0" maxlength = "1" value="<?php echo isset($email_address) ? $email_address : '' ?>">
        </div>
        <div class="form-group">
            <label for="password" class="control-label">Password</label>
            <input type="password" name="old_password" autofocus id="old_password" required class="form-control form-control-sm rounded-0" value="<?php echo isset($password) ? $password : '' ?>">
        </div>
...
<script>
    $(function(){
        $('#cashier-form').submit(function(e){
            ...
            $.ajax({
                url:'./sa_actions.php?a=update_Ccredentials',
                method:'POST',
                data:$(this).serialize(),
                dataType:'JSON',
                error:err=>{
