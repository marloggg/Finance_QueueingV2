...
Class Actions extends DBConnection{
    ...
    function sa_login(){
        extract($_POST);
        $sql = "SELECT * FROM superadmin_list where username = '{$username}' and `password` = '".md5($password)."' ";
        @$qry = $this->query($sql)->fetchArray();
        if(!$qry){
            $resp['status'] = "failed";
            $resp['msg'] = "Invalid username or password.";
    ...
    function save_add_admin(){
        ...
        @$check= $this->query("SELECT count(admin_id) as `count` FROM administrator_list where `username` = '{$username}' ".($id > 0 ? " and admin_id != '{$id}' " : ""))->fetchArray()['count'];
        if(@$check> 0){
            $resp['status'] = 'failed';
            ...
            $resp['msg'] = "Password should be at least 8 characters in length and should include at least one upper case letter, one number, and one special character.";    
                ...
                $resp['msg'] = "Password do not Match!";
                    ...
                    if(empty($id)){
                        $sql = "INSERT INTO `administrator_list` {$data}";
                        @$save = $this->query($sql);
                        if($save){
                            $resp['status'] = 'success';
                            $resp['type'] = 'success';
    ...
    function update_SAcredentials(){
        ...
        if(!empty($password) && md5($old_password) != $_SESSION['password']){
            ...
            $sql = "UPDATE `superadmin_list` set {$data} where superadmin_id = '{$_SESSION['superadmin_id']}'";
            @$save = $this->query($sql);
            if($save){
                $resp['status'] = 'success';
    ...
    function update_Acredentials(){
        ...
        if(empty($password)){
            ...
            $resp['msg'] = "Password should be at least 8 characters in length and should include at least one upper case letter, one number, and one special character.";    
                ...
                $sql = "UPDATE `administrator_list` set {$data} where admin_id = '{$_POST['id']}'";
                @$save = $this->query($sql);
                if($save){
                    $resp['status'] = 'success';
                    $resp['type'] = 'success';
    ...
    function delete_user(){
        ...
        @$delete = $this->query("DELETE FROM `administrator_list` where rowid = '{$id}'");
        if($delete){
            $resp['status']='success';
            $_SESSION['flashdata']['type'] = 'success';
        ...
        if($res){
            ...
            if($is_logged){
                ...
                if($delete){
                    $resp['status']='success';
                    $_SESSION['flashdata']['type'] = 'success';
                    $_SESSION['flashdata']['msg'] = 'Terminal successfully deleted.';
                }else{
                    $resp['status']='failed';
                    $resp['error']=$this->lastErrorMsg();
                }
...
switch($a){
    ...
    break;
    case 'sa_logout':
        echo $action->sa_logout();
    break;
    case 'save_add_admin':
        echo $action->save_add_admin();
    break;
    case 'update_SAcredentials':
        echo $action->update_SAcredentials();
    break;
