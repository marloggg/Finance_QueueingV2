    ...
    <form action="" id="teller-form">
        ...
        <div class="form-group">
            ...
            <select name="log_status" id="log_status" class="form-select form-select-sm rounded-0" required>
                <option value="1" <?php echo isset($log_status) && $status == 1 ? 'selected' : '' ?>>In-Use</option>
                <option value="2" <?php echo isset($log_status) && $status == 2 ? 'selected' : '' ?>>Not In-Use</option>
            </select>
        </div>
        <div class="form-group">
...
<script>
    $(function(){
        $('#teller-form').submit(function(e){
            ...
            $.ajax({
                url:'./sa_actions.php?a=update_teller',
                method:'POST',
                data:$(this).serialize(),
                dataType:'JSON',
                error:err=>{
